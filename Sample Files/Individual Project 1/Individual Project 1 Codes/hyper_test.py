...
# Accuracy on training data with the best model
X_train_pred = best_model.predict(train_X)
training_data_accuracy = accuracy_score(X_train_pred, train_y)
print(f"Training accuracy with the best model: {training_data_accuracy}")
...
submission_df.to_csv("y_predict_hyper.csv", index=False)
